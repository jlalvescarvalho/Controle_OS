...
begin
  try
  if MessageDlg('Atenção. Deseja realmente restaurar ? Isso irá subtituir tudo feito até agora.', mtConfirmation, mbYesNo, 0) = mrYes then
  begin
    ...
    NewBackupName := ChangeFileExt(BackupPath,'- '+ FormatDateTime('yyyymmdd_hhnnss', Now) + '.FDB');

    if not RenameFile(BackupPath, NewBackupName) then
      raise Exception.Create('Não foi possível renomear o banco de dados existente.');
